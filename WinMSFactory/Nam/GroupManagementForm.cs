    ...
    public partial class GroupManagementForm : ListForm
    {
        ...
        private void btnDelete_Click(object sender, EventArgs e) // 삭제
        {
            ...
            foreach(DataGridViewRow row in dgv.Rows)
            {
                ...
                if (chk.Value == null)
                {
                    NullCheck.Add(chk.Value);
                    continue;
                }
                    
                if ((bool)chk.Value == true)
                    DeleteNumbers.Add(dgv[1, row.Index].Value.ToInt());
            }

            if(DeleteNumbers.Count == 0) // 체크가 안되어있을 때
            {
                int DeleteNum = dgv.SelectedRows[0].Cells[1].Value.ToInt();
            ...
            else
            {
                ...
                if (MessageBox.Show("정말로 삭제하시겠습니까?", "", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    foreach(int numbers in DeleteNumbers)
                    {
