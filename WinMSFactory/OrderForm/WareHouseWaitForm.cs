...
namespace WinMSFactory
{
    public partial class WareHouseWaitForm : ListListForm
    {
        ...
        private void WareHouseWaitForm_Load(object sender, EventArgs e)
        {
            ...
            dgv.AddNewColumns("발주번호", "order_no", 80, true, true, false, LeftAlign);
            dgv.AddNewColumns("순서", "order_seq", 55, false, true, false, LeftAlign);
            ...
            dgvDetail.AddNewColumns("순서", "order_seq", 55, true, true, false, LeftAlign);
            dgvDetail.AddNewColumns("창고", "storage_name", 100, true, true, false, LeftAlign);
            dgvDetail.AddNewColumns("품목", "product_id", 55, true, true, false, LeftAlign);
            ...
            cboCompany.ComboBinding(comService.GetCompanyByType("매입처"), "company_id", "company_name", "전체");
        ...
        private void Search(object sender, EventArgs e)
        {
            if (((MainForm)this.MdiParent).ActiveMdiChild == this)
            {
                ...
                pList = service.GetWareHouseByDate(fromDate, toDate, searchGubun);
                              
                if (searchGubun > 0)
                {
                    pList = (from item in pList
                             where item.company_id == searchGubun
                             select item).ToList();
                }               
                dgv.DataSource = null;
